---
name: release-github
description: Crea un nuevo release de Quick Explorer (SpeedExplorer) en GitHub: auditoría git, bump de versión en package.json/Cargo.toml/tauri.conf.json, CHANGELOG.md, commit + tag v*, push y gh release create con el MSI y el ejecutable portable. Use when the user asks to crear/publicar/generar/hacer un release, una release, publicar una versión nueva, "release vX.Y.Z", "sacar versión" o "subir versión" de la app.
---
...
- `gh` CLI autenticado: verificar con `gh auth status` antes de continuar
